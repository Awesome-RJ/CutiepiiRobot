...
from Cutiepii_Robot.utils.tempmail import TempMail
from Cutiepii_Robot import telethn, mongodb as db, APP_HASH as TEMP_MAIL_KEY
        ...
        for c in gmail:
            if event.sender_id == c["user"]:
                ...
                if not int(((datetime.now() - ttime)).total_seconds()) > 86400:  # 24 hrs
                    ...
                    return
                msg = await telethn.send_message(event.chat_id, "Working ...")
                msgid = msg.id
                await telethn.edit_message(
                    event.chat_id,
                    msgid,
    ...
    try:
        ...
        for c in gmail:
            if event.sender_id == c["user"]:
                ...
                index = 0
                chatid = event.chat_id
                msg = await telethn.send_message(chatid, "Loading ...")
                msgid = msg.id
                await telethn.edit_message(
                    chatid,
                    msgid,
...
@telethn.on(events.CallbackQuery(pattern=r"stopcheckinbox(\-(.*))"))
async def _(event):
    if not event.is_private:
        return
    ...
    if not event.sender_id == sender:
        ...
        return
    await telethn.edit_message(chatid, msgid, "Thanks for using Julia ♥️")


@telethn.on(events.CallbackQuery(pattern=r"startcheckinbox(\-(.*))"))
async def _(event):
    if not event.is_private:
        return
    tata = event.pattern_match.group(1)
    ...
    if type(mails) is dict:
        for key, value in mails.items():
            if value == "There are no emails yet":
                await telethn.edit_message(chatid, msgid, "There are no emails yet.")
                return
    header = f"**#{num} **"
    ...
    if attch > 0:
        ...
        for i in gg:
            ...
            await telethn.send_file(chatid, file=fname)
            os.remove(fname)


@telethn.on(events.CallbackQuery(pattern=r"checkinboxprev(\-(.*))"))
async def _(event):
    if not event.is_private:
        return
    tata = event.pattern_match.group(1)
...
@telethn.on(events.CallbackQuery(pattern=r"checkinboxnext(\-(.*))"))
async def _(event):
    if not event.is_private:
        return
    tata = event.pattern_match.group(1)
...
@telethn.on(events.CallbackQuery(pattern=r"refreshinbox(\-(.*))"))
async def _(event):
    if not event.is_private:
        return
    tata = event.pattern_match.group(1)
...
@telethn.on(events.CallbackQuery(pattern=r"dontremoveoldemail(\-(.*))"))
async def _(event):
    if not event.is_private:
        return
    tata = event.pattern_match.group(1)
    ...
    if not event.sender_id == sender:
        await event.answer("You haven't send that command !")
        return
    await telethn.edit_message(chatid, msgid, "Okay !\nNot a problem 😉")


@telethn.on(events.CallbackQuery(pattern=r"removeoldemail(\-(.*))"))
async def _(event):
    if not event.is_private:
        return
    tata = event.pattern_match.group(1)
    ...
    await telethn.edit_message(chatid, msgid, f"Your new temporary email is: {email}")