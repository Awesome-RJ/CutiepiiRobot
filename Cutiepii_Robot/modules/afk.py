        ...
        for ent in entities:
            if ent.type == MessageEntity.TEXT_MENTION:
                ...
                try:
                    ...
                    print("Cutie Cutie! Error: Could not fetch userid {} for AFK module".
                          format(user_id))
                    return
...
def __user_info__(user_id):
    ...
    if is_afk:
        since_afk = get_readable_time((time.time() - float(REDIS.get(f'afk_time_{user_id}'))))
        text = "<i>Cutie Cutie! This user is currently afk (away from keyboard).</i>"
        text += f"\n<i>Since: {since_afk}</i>"
       
    else:
        text = "<i>Cutie Cutie! This user is currently isn't afk (away from keyboard).</i>"
    return text
