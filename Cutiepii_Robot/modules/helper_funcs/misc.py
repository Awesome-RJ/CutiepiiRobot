...
from Cutiepii_Robot import LOAD, NO_LOAD
    ...
    if len(pairs) > 7:
        pairs = pairs[modulo_page * 7:7 * (modulo_page + 1)] + [
            (EqInlineKeyboardButton("↖️", callback_data="{}_prev({})".format(prefix, modulo_page)),
                EqInlineKeyboardButton("【Ｂａｃｋ】", callback_data="cutiepii_back"),
             EqInlineKeyboardButton("↘️", callback_data="{}_next({})".format(prefix, modulo_page)))]

    else:
        pairs += [[EqInlineKeyboardButton("【Ｂａｃｋ】", callback_data="cutiepii_back")]]
