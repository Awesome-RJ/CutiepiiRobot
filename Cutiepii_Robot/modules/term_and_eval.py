...
from Cutiepii_Robot import pgram, OWNER_ID
...
@pgram.on_message(filters.user(OWNER_ID) & filters.command("eval"))
...
@pgram.on_message(filters.user(OWNER_ID) & filters.command("term"))
...
""".format(err))
    ...
    if output:
        if len(output) > 4096:
            with open("Cutiepii_Robot/output.txt", "w+") as file:
                file.write(output)
